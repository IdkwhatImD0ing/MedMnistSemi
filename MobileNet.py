...
labeled_x_train, labeled_x_val, labeled_y_train, labeled_y_val = train_test_split(labeled_x_train, labeled_y_train, test_size = 0.1)
...
semi_model.compile(optimizer=keras.optimizers.RMSprop(learning_rate=0.005), metrics=["accuracy"])
semi_model.build(input_shape=(None, 64, 64, 3))
semi_model.summary()

history = semi_model.fit(train_dataset, epochs= NUM_EPOCHS, validation_data = (labeled_x_val, labeled_y_val))
